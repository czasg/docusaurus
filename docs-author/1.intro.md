...
**背景：**    
&nbsp;&nbsp;&nbsp;&nbsp;云查引擎，是一款定位于解决终端引擎扫描速度慢、病毒规则更新慢等瓶颈问题的产品，除此之外，还支持快速上下线规则、定向、多元打击等能力。当前云查引擎包含云查、KV-DB、云查数据加工三大板块。        
...
&nbsp;&nbsp;&nbsp;&nbsp;2020年左右，云查业务上量。为了后续扩展，在云查架构上做出调整，整体将云查拆分为三大板块：        
* 云查：提供HTTP业务查询接口       
* KV-DB：提供gRPC数据查询接口     
* 云查数据加工：采集多方数据源，实时数据清洗，提供数据更新    

&nbsp;&nbsp;&nbsp;&nbsp;2021年以后，业务持续上量，当前数据同步方案存在较大缺陷，导致线上数据更新异常。新KV-DB研发正式启动，旨在解决弱网环境下数据可靠性问题，以保障异地多活场景下的数据最终一致性。      
&nbsp;&nbsp;&nbsp;&nbsp;2022年新KV-DB正式上线投入使用。     
...
* 合理设计业务分流、本地缓存、gRPC等一系列方案，抗住高峰期百万并发压力，日活二十亿+。
* Redis Key 优化，raw编码转embstr编码，减少动态字符串类型内存分配与内存碎片。
* GOMAXPROCS定位及调优。

**补充：**     
* 当前体系已经逐渐庞大，后续已考虑采用**binlog**方式实现异地多活。

### 误报测试系统
**关键字：**TB级快速扫描、分布式存储
**简介：**
对外发布病毒规则前，需要进行一轮内部防误报测试。以便减少对对外的误报几率。
**职责：**
* 负责误报测试系统设计与研发。
**亮点：**
...
**关键字：**分布式下载
**简介：**
为了扩大样本库，需要支持多元的样本来源，包括厂商交换、应用市场爬虫等。  
通用下载中心，基于 ari2 搭建的，支持http、ftp等多种下载协议。
**职责：**
* 负责通用下载服务的维护
**亮点：**
...
**关键字：**
**简介：**
基于卷积神经网络搭建数字字符型验证码识别平台，提供验证码识别接口。针对图片的难度，可快速适配多种卷积模型来进行训练，对简易验证码实现快速训练，对复杂验证码实现精度更高的训练。     
